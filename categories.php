    ...
    <section class="container my-5 main-section">
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4">
            ...
            while ($category = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo '<img src="https://via.placeholder.com/300x200" class="card-img-top" alt="' . htmlspecialchars($category['name']) . '">';
                echo '<div class="card-body">';
                echo '<h5 class="card-title">' . htmlspecialchars($category['name']) . '</h5>';
                echo '<p class="card-text">Find items in ' . htmlspecialchars($category['name']) . ' near you.</p>';
                echo '<a href="browse_ads.php?cat=' . htmlspecialchars($category['name']) . '" class="btn btn-success">Explore</a>';
                echo '</div></div></div>';
            }
            $location = filter_input(INPUT_GET, 'location', FILTER_SANITIZE_STRING);
            if (!$location) {
                echo '<p class="text-center">Please allow location access to see local categories.</p>';
            }
            ?>
        </div>
    </section>
